...
$PATHS=array(
"path_to_calendar" => "/homepages/30/d162063315/htdocs/secondstreet/master/cal/cl_files/",
"path_to_calendar_img" => "/homepages/30/d162063315/htdocs/secondstreet/master/cal/cl_files/img/",
"WEB_path_to_calendar_img" => "http://www.secondstreetcottages.com/master/cal/cl_files/img/",
"path_to_data" => "/homepages/30/d162063315/htdocs/secondstreet/master/cal/cl_files/data/"
);

// Please, do not change anything below this line
// ----------------------------------------------
$notReplace = array('footer_file','header_file','event_list_templ');
foreach($HTTP_POST_VARS as $k=>$v){
	if(!is_array($v)){
		if(!in_array($k,$notReplace)){
			${urldecode($k)}=preg_replace("/[^a-z,A-Z,0-9_\.\/]/", '', urldecode($v));
		}
	}
}
...
class calendar{
	...
	*/
	function show_header(){
		...
		<td style="color: white;" <?php echo  $this->style_bg($this->a_look['header_bg_color'],'left')?> width="5%" nowrap><?php if($this->a_template['show_year']) echo $this->put_href("<img src=".$this->put_arrows('py')." border=0 alt='previous year'>",'','',"CLy".$this->s_calendar_index."=".($this->a_selected_date['y']-1)).'&nbsp;';echo $this->put_href("<img src=".$this->put_arrows('p')." border=0 alt='prev month'>",'','',($this->a_selected_date['m']-1<1?"CLm".$this->s_calendar_index."=12&CLy".$this->s_calendar_index."=".($this->a_selected_date['y']-1):"CLm".$this->s_calendar_index."=".($this->a_selected_date['m']-1)));?></td>
	...
	function show_v() {
		...
		if($this->a_template['weeks_location'] == 1) {
			...
			if($this->a_template['days_location'] == 1)
				echo "<td color='white' ".$this->style_bg($this->a_look['header2_bg_color']).">&nbsp;</td>";
			for($j = 0; $j < $rows; $j++) {
				$d = $j * 7 + 1 + $__s - $dwf;
			...
			if($this->a_template['days_location']==2)
				echo "<td ".$this->style_bg($this->a_look['header2_bg_color']).">&nbsp;  HELLO !!!!!!!!!!!!!!!!!</td>";
			echo "</tr>";
		}
		for($i = $__s; $i <= $__e; $i++) {
			echo "<tr>";
	...
	*/
	function put_href($str, $link, $title, $params, $adds = '', $only_this = 0, $font = '') {
		...
		list($_GL) = split('\?', $HTTP_SERVER_VARS['REQUEST_URI']);
		$link = $link ? $link : $_GL;
		$_p = split("&", $params);
		foreach($_p as $k => $v) {
			list($ke, $va) = split("=", $v);
		...
		if(!$this->is_Admin)$p = array_merge($HTTP_POST_VARS, $HTTP_GET_VARS, $PAR);
		else $p = array_merge($HTTP_GET_VARS, $PAR);

		foreach($p as $k =>$v)$vars[] = "$k=".preg_replace("/[^a-z,A-Z,0-9_\.\/]/", '', $v);
...
$js_pos[] = "set_pos(".$v['v']->time_start['h'].",".$v['v']->time_start['m'].",".$v['v']->time_end['h'].",".$v['v']->time_end['m'].",'event".$v['v']->id."','et',".$v['x_s'].",".$v['width'].");";
	...
	*/
	function show_week(){
		...
		?>
		<tr>			<td align="center"  bgcolor="#DBEAF5" valign="top" width="50" height="30"><?php echo $_h?></td>
